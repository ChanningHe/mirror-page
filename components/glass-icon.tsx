...
const gradientMapping: Record<string, string> = {
  ...
  violet: 'linear-gradient(hsl(273, 90%, 55%), hsl(258, 90%, 55%))',
  rose: 'linear-gradient(hsl(343, 90%, 55%), hsl(328, 90%, 55%))'
};
...
const GlassIcons: React.FC<GlassIconsProps> = ({ items, className }) => {
  ...
  return (
    <div className={`grid gap-[3em] grid-cols-2 md:grid-cols-3 mx-auto py-[2em] overflow-visible ${className || ''}`}>
...
// Simple wrapper for single icon usage
export interface SingleGlassIconProps {
  icon: React.ReactElement;
  color?: string;
  label?: string;
  className?: string;
...
export const SingleGlassIcon: React.FC<SingleGlassIconProps> = ({ 
  icon, 
  color = 'blue', 
  label = '',
  className 
}) => {
  return <GlassIcons items={[{ icon, color, label }]} className={className} />;
};
